import{p as q,b as z,r as f,c as y}from"./index-cRxWXIxg.js";function v(c){try{const d=JSON.stringify(c);return btoa(unescape(encodeURIComponent(d)))}catch{return""}}function C(c,d="exam_section",u,b){typeof d=="function"&&(b=u,u=d,d="exam_section");const s=document.createElement("div");s.className="modal-overlay fade-in",s.style.position="fixed",s.style.inset="0",s.style.background="rgba(0, 0, 0, 0.6)",s.style.backdropFilter="blur(4px)",s.style.zIndex="10000",s.style.display="flex",s.style.alignItems="center",s.style.justifyContent="center",s.style.padding="20px";let o=1,p=[],g=null,a=null;const _=`category_name,tag_names,passage_en,passage_hi,passage_image_url,question_en,question_hi,image_url,optionA_en,optionB_en,optionC_en,optionD_en,optionA_hi,optionB_hi,optionC_hi,optionD_hi,answer,explanation_en,explanation_hi,explanation_image_url,difficulty
...
2. अध्यक्ष अन्यथा निर्देश दे सकते हैं।`,image_url:"https://example.com/question_diagram.png",options_en:["1 only","2 only","Both 1 and 2","Neither 1 nor 2"],options_hi:["केवल 1","केवल 2","1 और 2 दोनों","न तो 1 और न ही 2"],options_images:["","","",""],correct_option_index:2,explanation_en:"Both statements correctly reflect Rule 32 (Question Hour).",explanation_hi:"दोनों कथन नियम 32 (प्रश्न काल) को सही रूप से दर्शाते हैं।",explanation_image_url:"https://example.com/explanation_chart.png",difficulty:"medium"}],null,2),t=document.createElement("div");t.className="card",t.style.width="100%",t.style.maxWidth="820px",t.style.maxHeight="90vh",t.style.overflowY="auto",t.style.padding="28px",t.style.background="var(--card-bg)",t.style.borderRadius="20px",t.style.boxShadow="0 25px 50px -12px rgba(0, 0, 0, 0.25)";function m(){t.innerHTML=`
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid var(--border-color); padding-bottom: 14px;">
        <div>
          ...
          <p style="font-size: 0.85rem; color: var(--text-muted); margin-top: 2px;">Step ${o} of 3 — Import KaTeX Math, Images & Passages</p>
        </div>
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 24px;">
        <div style="flex: 1; height: 6px; border-radius: 3px; background: ${o>=1?"var(--primary)":"var(--border-color)"};"></div>
        <div style="flex: 1; height: 6px; border-radius: 3px; background: ${o>=2?"var(--primary)":"var(--border-color)"};"></div>
        <div style="flex: 1; height: 6px; border-radius: 3px; background: ${o>=3?"var(--primary)":"var(--border-color)"};"></div>
      </div>
    ...
    `}function E(){var n;if((n=t.querySelector("#close-wizard-btn"))==null||n.addEventListener("click",()=>{document.body.removeChild(s),b&&b()}),o===1){const e=t.querySelector("#drop-zone"),r=t.querySelector("#file-input");e.addEventListener("click",()=>r.click()),e.addEventListener("dragover",i=>{i.preventDefault(),e.style.background="var(--primary-border)"}),e.addEventListener("dragleave",()=>{e.style.background="var(--primary-light)"}),e.addEventListener("drop",i=>{i.preventDefault(),e.style.background="var(--primary-light)",i.dataTransfer.files.length>0&&x(i.dataTransfer.files[0])}),r.addEventListener("change",i=>{i.target.files.length>0&&x(i.target.files[0])}),t.querySelector("#dl-csv-sample").addEventListener("click",()=>h("ssc_questions_template.csv",_)),t.querySelector("#dl-json-sample").addEventListener("click",()=>h("ssc_questions_template.json",k))}if(o===2){t.querySelector("#btn-back-step1").addEventListener("click",()=>{o=1,m()});const e=t.querySelector("#btn-next-step3");e&&!e.disabled&&e.addEventListener("click",()=>{o=3,m()})}o===3&&(t.querySelector("#btn-back-step2").addEventListener("click",()=>{o=2,m()}),t.querySelector("#btn-confirm-import").addEventListener("click",async()=>{const e=t.querySelector("#btn-confirm-import");e.disabled=!0,e.innerHTML="Importing Base64 Payload... ⏳";try{const r=d==="quiz"?`/quizzes/${c}/questions/bulk`:d==="question_bank"?"/exams/questions/bulk":`/exams/sections/${c}/questions/bulk`,i={questions:p,encodedPayload:v(p),section_id:d==="question_bank"?c:null};console.log("[DEBUG CLIENT] Sending Base64 bulk import request:",{endpoint:r,targetId:c,targetType:d,payloadLength:i.encodedPayload.length});const l=await f(r,{method:"POST",body:JSON.stringify(i)});console.log("[DEBUG CLIENT] Bulk import response:",l),y.invalidate(`questions_${c}`),y.invalidate("quizzes"),y.invalidate("questions"),alert(l.message),document.body.removeChild(s),u&&u(l)}catch(r){console.error("[DEBUG CLIENT] Bulk import error:",r),alert(r.message||"Bulk import failed."),e.disabled=!1,e.innerHTML="Execute Bulk Import (Base64 Encoded) 🚀"}}))}async function x(n){const e=new FileReader;e.onload=async r=>{try{const i=r.target.result;if(n.name.endsWith(".json")?p=q(i):p=z(i),p.length===0){g="No valid questions could be extracted from the file.",m();return}g=null,a=null;try{const l=await f("/exams/questions/validate-bulk",{method:"POST",body:JSON.stringify({encodedPayload:v(p)})});l.valid||(a={missingCategories:l.missingCategories||[],missingTags:l.missingTags||[]})}catch(l){console.warn("Pre-validation failed:",l)}o=2,m()}catch(i){g=i.message,m()}},e.readAsText(n)}function h(n,e){const r=new Blob([e],{type:"text/plain"}),i=URL.createObjectURL(r),l=document.createElement("a");l.href=i,l.download=n,l.click(),URL.revokeObjectURL(i)}s.appendChild(t),document.body.appendChild(s),m()}export{C as renderBulkUploadModal};
