import{p as z,a as q}from"./csvJsonParser-Cmc2jVv8.js";import{r as v,e as y}from"./index-BrA-5OY9.js";function h(d){try{const l=JSON.stringify(d);return btoa(unescape(encodeURIComponent(l)))}catch{return""}}function B(d,l="exam_section",u,b){typeof l=="function"&&(b=u,u=l,l="exam_section");const r=document.createElement("div");r.className="modal-overlay fade-in",r.style.position="fixed",r.style.inset="0",r.style.background="rgba(0, 0, 0, 0.6)",r.style.backdropFilter="blur(4px)",r.style.zIndex="10000",r.style.display="flex",r.style.alignItems="center",r.style.justifyContent="center",r.style.padding="20px";let s=1,p=[],g=null,c=null;const _=`category_name,tag_names,passage_en,passage_hi,passage_image_url,question_en,question_hi,image_url,optionA_en,optionB_en,optionC_en,optionD_en,optionA_hi,optionB_hi,optionC_hi,optionD_hi,answer,explanation_en,explanation_hi,explanation_image_url,difficulty
...
"Mathematics","Algebra,Equations","","Solve $E = mc^2$ for $m$.","समीकरण $E = mc^2$ में $m$ का मान बताएं।","","m = E/c^2","m = Ec^2","m = c^2/E","m = E - c^2","","","","",0,"Dividing both sides by c^2.","दोनों पक्षों को c^2 से विभाजित करना।","",hard`,w=JSON.stringify([{category_name:"General Science",tag_names:["Physics","SSC CGL"],passage_text_en:"Rule 32: Unless the Speaker otherwise directs, the first hour of every sitting shall be available for questions.",passage_text_hi:"नियम 32: जब तक अध्यक्ष अन्यथा निर्देश न दें, प्रत्येक बैठक का प्रथम घंटा प्रश्नों के लिए उपलब्ध होगा।",passage_image_url:"https://example.com/passage_diagram.png",question_text_en:`Consider the following statements regarding Rule 32:
...
2. अध्यक्ष अन्यथा निर्देश दे सकते हैं।`,image_url:"https://example.com/question_diagram.png",options_en:["1 only","2 only","Both 1 and 2","Neither 1 nor 2"],options_hi:["केवल 1","केवल 2","1 और 2 दोनों","न तो 1 और न ही 2"],options_images:["","","",""],correct_option_index:2,explanation_en:"Both statements correctly reflect Rule 32 (Question Hour).",explanation_hi:"दोनों कथन नियम 32 (प्रश्न काल) को सही रूप से दर्शाते हैं।",explanation_image_url:"https://example.com/explanation_chart.png",difficulty:"medium"}],null,2),i=document.createElement("div");i.className="card",i.style.width="100%",i.style.maxWidth="820px",i.style.maxHeight="90vh",i.style.overflowY="auto",i.style.padding="28px",i.style.background="var(--card-bg)",i.style.borderRadius="20px",i.style.boxShadow="0 25px 50px -12px rgba(0, 0, 0, 0.25)";function m(){i.innerHTML=`
    ...
    `}function S(){const o=c&&c.missingCategories||[],t=c&&(c.missingTags||c.newTagsToCreate)||[],n=o.length>0;return`
      <div style="margin-bottom: 20px;">
        ...
        ${t.length>0?`
          <div style="background: var(--primary-light); border: 1px solid var(--primary-border); border-radius: 12px; padding: 14px; margin-bottom: 16px;">
            ...
            <div style="display:flex; gap:6px; flex-wrap:wrap;">
              ${t.map(e=>`<span class="badge-tag" style="background:var(--bg-color); border:1px solid var(--border-color); font-weight:700;">🏷️ ${e}</span>`).join("")}
            </div>
          </div>
        `:""}
        ...
        <div style="overflow-x: auto; max-height: 280px; border: 1px solid var(--border-color); border-radius: 10px;">
          <table class="custom-table" style="width: 100%; font-size: 0.85rem;">
            ...
            <tbody>
              ${p.map((e,a)=>`
                <tr>
                  <td style="font-weight: 700;">${a+1}</td>
    ...
    `}function $(){let o="Exam Section";return l==="quiz"&&(o="Practice Quiz"),l==="question_bank"&&(o="Master Question Repository"),`
      <div style="text-align: center; padding: 20px 0; margin-bottom: 20px;">
        ...
        <p style="color: var(--text-muted); font-size: 0.9rem;">Questions will be added into ${o} ${d?`#${d}`:""} with multi-language support, KaTeX math & embedded image URLs.</p>
      </div>
    ...
    `}function E(){var o;if((o=i.querySelector("#close-wizard-btn"))==null||o.addEventListener("click",()=>{document.body.removeChild(r),b&&b()}),s===1){const t=i.querySelector("#drop-zone"),n=i.querySelector("#file-input");t.addEventListener("click",()=>n.click()),t.addEventListener("dragover",e=>{e.preventDefault(),t.style.background="var(--primary-border)"}),t.addEventListener("dragleave",()=>{t.style.background="var(--primary-light)"}),t.addEventListener("drop",e=>{e.preventDefault(),t.style.background="var(--primary-light)",e.dataTransfer.files.length>0&&x(e.dataTransfer.files[0])}),n.addEventListener("change",e=>{e.target.files.length>0&&x(e.target.files[0])}),i.querySelector("#dl-csv-sample").addEventListener("click",()=>f("ssc_questions_template.csv",_)),i.querySelector("#dl-json-sample").addEventListener("click",()=>f("ssc_questions_template.json",w))}if(s===2){i.querySelector("#btn-back-step1").addEventListener("click",()=>{s=1,m()});const t=i.querySelector("#btn-next-step3");t&&!t.disabled&&t.addEventListener("click",()=>{s=3,m()})}s===3&&(i.querySelector("#btn-back-step2").addEventListener("click",()=>{s=2,m()}),i.querySelector("#btn-confirm-import").addEventListener("click",async()=>{const t=i.querySelector("#btn-confirm-import");t.disabled=!0,t.innerHTML="Importing Base64 Payload... ⏳";try{const n=l==="quiz"?`/quizzes/${d}/questions/bulk`:l==="question_bank"?"/exams/questions/bulk":`/exams/sections/${d}/questions/bulk`,e={questions:p,encodedPayload:h(p),section_id:l==="question_bank"?d:null};console.log("[DEBUG CLIENT] Sending Base64 bulk import request:",{endpoint:n,targetId:d,targetType:l,payloadLength:e.encodedPayload.length});const a=await v(n,{method:"POST",body:JSON.stringify(e)});console.log("[DEBUG CLIENT] Bulk import response:",a),y.invalidate(`questions_${d}`),y.invalidate("quizzes"),y.invalidate("questions"),alert(a.message),document.body.removeChild(r),u&&u(a)}catch(n){console.error("[DEBUG CLIENT] Bulk import error:",n),alert(n.message||"Bulk import failed."),t.disabled=!1,t.innerHTML="Execute Bulk Import (Base64 Encoded) 🚀"}}))}async function x(o){const t=new FileReader;t.onload=async n=>{try{const e=n.target.result;if(o.name.endsWith(".json")?p=z(e):p=q(e),p.length===0){g="No valid questions could be extracted from the file.",m();return}g=null,c=null;try{const a=await v("/exams/questions/validate-bulk",{method:"POST",body:JSON.stringify({encodedPayload:h(p)})});a.valid||(c={missingCategories:a.missingCategories||[],missingTags:a.missingTags||[]})}catch(a){console.warn("Pre-validation failed:",a)}s=2,m()}catch(e){g=e.message,m()}},t.readAsText(o)}function f(o,t){const n=new Blob([t],{type:"text/plain"}),e=URL.createObjectURL(n),a=document.createElement("a");a.href=e,a.download=o,a.click(),URL.revokeObjectURL(e)}r.appendChild(i),document.body.appendChild(r),m()}export{B as renderBulkUploadModal};
