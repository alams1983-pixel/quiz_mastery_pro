import{a as $,g as I,b as k,d as lt}from"./index-BJvuVE6r.js";import{r as R}from"./richContent-B2bHBeKP.js";function pt(q,Q,T){const t=document.createElement("div");t.className="view-container",t.innerHTML=`
  ...
  `;let y={title:"Mastery Practice",description:""},o=[],d=[],v=1,p={},f={},g=0,w=0,m=0,b=null,z=!1,i=null,C=[],D=0,x={},M=null;const N=t.querySelector("#startScreen"),H=t.querySelector("#activeScreen"),P=t.querySelector("#completionScreen"),Z=t.querySelector("#startQuizBtn"),F=t.querySelector("#timerDisplay"),K=t.querySelector("#masteryStatus"),X=t.querySelector("#progressFill"),tt=t.querySelector("#qText"),B=t.querySelector("#qImg"),E=t.querySelector("#optionsContainer"),_=t.querySelector("#feedback"),et=t.querySelector("#fbHead"),O=t.querySelector("#fbCorrectAnswer"),W=t.querySelector("#fbExplain"),j=t.querySelector("#nextBtnContainer"),rt=t.querySelector("#nextQuestionBtn"),G=t.querySelector("#guestNoticeBanner"),U=t.querySelector("#guestSignInBtn");function V(e){const r=String(Math.floor(e/60)).padStart(2,"0"),n=String(e%60).padStart(2,"0");return`${r}:${n}`}function A(e){for(let r=e.length-1;r>0;r--){const n=Math.floor(Math.random()*(r+1));[e[r],e[n]]=[e[n],e[r]]}return e}async function it(){try{Q&&Q.isWeakArea?(y={title:"Weak Areas Targeted Quiz",description:"Personalized practice focusing on weak questions"},o=Q.questions):(y=(await k.getQuiz(q)).quiz,o=(await k.getQuestions(q)).questions),t.querySelector("#quizTitleHeader").textContent=y.title,t.querySelector("#quizDescHeader").textContent=y.description||"Practice & repetition quiz mode.",t.querySelector("#totalQtyDisplay").textContent=o.length}catch(e){alert("Error loading quiz: "+e.message),T("dashboard")}}function st(){const e=t.querySelector('input[name="mastery"]:checked');v=e?parseInt(e.value,10):1,p={},f={},x={},g=0,w=0,m=0,z=!1,o.forEach(r=>{p[r.id]=0,f[r.id]=0,x[r.id]=0}),d=o.map(r=>r.id),A(d)}Z.addEventListener("click",()=>{st(),N.style.display="none",H.style.display="block",m=0,F.textContent="00:00",b&&clearInterval(b),b=setInterval(()=>{m++,F.textContent=V(m)},1e3),Y(),L()});function L(){if(d.length===0){ot();return}const e=Math.floor(Math.random()*d.length),r=d[e];if(i=o.find(s=>s.id===r),!i){d.splice(e,1),L();return}z=!1,D=Date.now(),tt.innerHTML=R(i.question_text),i.image_path?(B.src=`/api/images/${i.image_path}`,B.style.display="block"):B.style.display="none";const n=["A","B","C","D","E","F"];E.innerHTML="";const l=(i.options||[]).map((s,u)=>({text:s,origIdx:u}));C=A([...l]),C.forEach((s,u)=>{const a=document.createElement("button");a.className="option-btn",a.innerHTML=`<span class="opt-label">${n[u]}</span><span class="opt-text">${R(s.text)}</span>`,a.addEventListener("click",()=>nt(u)),E.appendChild(a)}),_.className="feedback",O.textContent="",W.innerHTML="",j.style.display="none",$(t.querySelector("#questionCard"))}async function nt(e){if(z)return;z=!0;const r=Math.max(1,Math.round((Date.now()-D)/1e3));x[i.id]=(x[i.id]||0)+r;const n=E.querySelectorAll(".option-btn");n.forEach(c=>c.classList.add("disabled-opt"));const l=C[e],s=l&&l.origIdx===i.correct_answer_index,u=i.options[i.correct_answer_index];if(n.forEach((c,h)=>{const S=C[h];S&&S.origIdx===i.correct_answer_index&&c.classList.add("correct-opt"),h===e&&!s&&c.classList.add("wrong-opt")}),s){if(p[i.id]=(p[i.id]||0)+1,g++,p[i.id]>=v){const c=d.indexOf(i.id);c!==-1&&d.splice(c,1)}}else f[i.id]=(f[i.id]||0)+1,w++,A(d);_.className=`feedback visible ${s?"correct":"wrong"}`,et.textContent=s?"✅ Correct!":"❌ Incorrect",O.textContent=s?"":`Correct Answer: ${u}`,W.innerHTML=R(i.explanation||""),$(_),Y(),j.style.display="block",I()&&k.logQuestion({question_id:i.id,quiz_id:q||i.quiz_id,is_correct:s,time_spent_sec:r,selected_option_index:l?l.origIdx:0}).catch(console.error)}rt.addEventListener("click",()=>{L()});function Y(){const e=o.length;let r=0;o.forEach(l=>{(p[l.id]||0)>=v&&r++});const n=e>0?r/e*100:0;X.style.width=`${n}%`,K.textContent=`Mastered: ${r} / ${e}`}async function ot(){b&&clearInterval(b);const e=o.length,r=g+w,n=r>0?Math.round(g/r*100):0;t.querySelector("#statTotal").textContent=e,t.querySelector("#statAttempts").textContent=r,t.querySelector("#statCorrect").textContent=g,t.querySelector("#statWrong").textContent=w,t.querySelector("#statAccuracy").textContent=n+"%",t.querySelector("#statTime").textContent=V(m);const l=t.querySelector("#summaryTableBody");l.innerHTML="";const s={};if(o.forEach((a,c)=>{const h=p[a.id]||0,S=f[a.id]||0,at=h+S;s[a.id]={correct:h,wrong:S,time_spent:x[a.id]||0};const J=document.createElement("tr");J.innerHTML=`
      ...
      `,l.appendChild(J)}),$(l),M={quiz_id:q||(o[0]?o[0].quiz_id:1),score:e,total_questions:e,accuracy_pct:n,time_taken_sec:m,mastery_level:v,details_json:s},I()){G.style.display="none";try{await k.saveQuizAttempt(M)}catch(a){console.error("Failed to save quiz attempt:",a)}}else G.style.display="block";H.style.display="none",P.style.display="block"}return U&&U.addEventListener("click",()=>{T("login")}),t.querySelector("#downloadPdfBtn").addEventListener("click",async()=>{const e=I()||{full_name:"Guest Student",email:"guest@example.com",role:"user"};await lt({user:e,quiz:y,attempt:M,questions:o})}),t.querySelector("#restartQuizBtn").addEventListener("click",()=>{P.style.display="none",N.style.display="block"}),t.querySelector("#backCatalogBtn").addEventListener("click",()=>{T("dashboard")}),it(),t}export{pt as renderQuizView};
