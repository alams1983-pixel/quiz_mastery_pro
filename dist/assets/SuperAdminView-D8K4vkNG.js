import{r as p}from"./index-w1j3NIg4.js";function C(){const t=document.createElement("div");return t.className="view-container fade-in",t.innerHTML=`
  ...
  `,setTimeout(()=>{w(t),y(t)},0),t}let u=[],m=[];async function y(t){try{const[s,a]=await Promise.all([p("/institutes"),p("/auth/users")]);u=s.institutes||[],m=a.users||[];let e=0,i=0;u.forEach(n=>{e+=parseInt(n.student_count||0,10),i+=parseInt(n.quiz_count||0,10)}),t.querySelector("#stat-institutes").textContent=u.length,t.querySelector("#stat-students").textContent=e,t.querySelector("#stat-quizzes").textContent=i,t.querySelector("#stat-users").textContent=m.length,g(t,u),f(t,m)}catch(s){console.error("Failed to load Super Admin data:",s)}}function g(t,s){const a=t.querySelector("#institutes-table-body");if(!s||s.length===0){a.innerHTML='<tr><td colspan="7" style="text-align: center; padding: 24px; color: var(--text-muted);">No coaching institutes found. Click "Create Coaching Institute" to add one.</td></tr>';return}a.innerHTML=s.map(e=>`
    <tr>
  ...
  `).join(""),a.querySelectorAll(".role-selector").forEach(e=>{e.addEventListener("change",async()=>{const i=e.dataset.id,n=e.value;try{await p(`/auth/users/${i}/role`,{method:"PUT",body:JSON.stringify({role:n})}),y(t)}catch(o){alert(o.message||"Error updating user role")}})})}function w(t){const s=t.querySelector("#tab-institutes"),a=t.querySelector("#tab-users"),e=t.querySelector("#section-institutes"),i=t.querySelector("#section-users");s.addEventListener("click",()=>{s.classList.add("active"),s.style.borderBottom="3px solid var(--primary)",s.style.color="var(--text-main)",a.classList.remove("active"),a.style.borderBottom="none",a.style.color="var(--text-muted)",e.style.display="block",i.style.display="none"}),a.addEventListener("click",()=>{a.classList.add("active"),a.style.borderBottom="3px solid var(--primary)",a.style.color="var(--text-main)",s.classList.remove("active"),s.style.borderBottom="none",s.style.color="var(--text-muted)",i.style.display="block",e.style.display="none"});const n=t.querySelector("#modal-create-inst"),o=t.querySelector("#btn-create-institute"),v=t.querySelector("#close-modal-inst"),x=t.querySelector("#cancel-modal-inst"),h=t.querySelector("#form-create-inst"),S=()=>{n.style.display="flex"},b=()=>{n.style.display="none",h.reset()};o.addEventListener("click",S),v.addEventListener("click",b),x.addEventListener("click",b),h.addEventListener("submit",async d=>{d.preventDefault();const r={name:t.querySelector("#inst-name").value.trim(),contact_email:t.querySelector("#inst-email").value.trim(),address:t.querySelector("#inst-address").value.trim(),admin_name:t.querySelector("#inst-admin-name").value.trim(),admin_email:t.querySelector("#inst-admin-email").value.trim(),admin_password:t.querySelector("#inst-admin-pass").value.trim()};try{const l=await p("/institutes",{method:"POST",body:JSON.stringify(r)});alert(`Institute "${r.name}" created successfully! Code: ${l.code}`),b(),y(t)}catch(l){alert(l.message||"Error creating institute.")}}),t.querySelector("#search-institutes").addEventListener("input",d=>{const r=d.target.value.toLowerCase(),l=u.filter(c=>c.name.toLowerCase().includes(r)||c.code.toLowerCase().includes(r));g(t,l)}),t.querySelector("#search-users").addEventListener("input",d=>{const r=d.target.value.toLowerCase(),l=m.filter(c=>c.full_name.toLowerCase().includes(r)||c.email.toLowerCase().includes(r));f(t,l)})}export{C as renderSuperAdminView};
