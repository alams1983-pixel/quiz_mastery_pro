import{r as z,a as Se,e as ke,h as ce}from"./index-CXmA39g4.js";import{n as G}from"./csvJsonParser-BYEbs1eI.js";function Ie(e,I={}){const b=document.createElement("div");b.className="view-container fade-in";const T=I.questionId||null,f=I.returnView||"exam-questions";return b.innerHTML=`
    <!-- Top Action Bar -->
  ...
  `,setTimeout(()=>{Le(b,e,T,f)},0),b}async function Le(e,I,b,T){let f="en",h=4,q=0;const v=new Map,n=new Map,ge=e.querySelector("#btn-editor-back"),me=e.querySelector("#btn-save-question"),ue=e.querySelector("#btn-save-next-question"),C=e.querySelector("#tab-lang-en"),O=e.querySelector("#tab-lang-hi"),K=e.querySelector("#section-lang-en"),ee=e.querySelector("#section-lang-hi"),te=e.querySelector("#section-passage-en"),ae=e.querySelector("#section-passage-hi"),ie=e.querySelector("#explanation-pane-en"),oe=e.querySelector("#explanation-pane-hi"),$=e.querySelector("#form-p-text-en"),B=e.querySelector("#form-p-text-hi"),R=e.querySelector("#form-q-text-en"),Q=e.querySelector("#form-q-text-hi"),A=e.querySelector("#form-q-exp-en"),D=e.querySelector("#form-q-exp-hi"),J=e.querySelector("#form-q-diff"),j=e.querySelector("#form-q-section"),W=e.querySelector("#form-q-global"),se=e.querySelector("#prev-lang-label"),ve=e.querySelector("#preview-diff-badge"),X=e.querySelector("#options-list-builder"),xe=e.querySelector("#btn-add-opt"),ye=e.querySelector("#btn-rem-opt");function F(t,l,a="",i){const s=document.createElement("div");s.className="image-picker-widget-box",s.dataset.field=t;const o=G(a);s.innerHTML=`
    ...
    `;const r=s.querySelector(".img-thumb-card"),g=s.querySelector(".img-thumb-preview"),x=s.querySelector(".img-type-badge"),y=s.querySelector(".img-path-txt"),L=s.querySelector(".btn-remove-picker-img"),m=s.querySelector(".img-picker-controls"),S=s.querySelector(".picker-url-input"),k=s.querySelector(".picker-file-input");return s.querySelector(".btn-browse-file").addEventListener("click",()=>k.click()),k.addEventListener("change",()=>{if(k.files[0]){const d=k.files[0];n.has(t)&&URL.revokeObjectURL(n.get(t));const u=URL.createObjectURL(d);n.set(t,u),v.set(t,d),g.src=u,x.textContent="📁 Local File (Pending Upload)",y.textContent=d.name+` (${(d.size/1024).toFixed(1)} KB)`,r.style.display="flex",m.style.display="none",i&&i(u)}}),S.addEventListener("input",()=>{const d=S.value.trim();d&&(n.has(t)&&(URL.revokeObjectURL(n.get(t)),n.delete(t)),v.delete(t),g.src=d,x.textContent="🌐 Image URL Attached",y.textContent=d,r.style.display="flex",m.style.display="none"),i&&i(d)}),L.addEventListener("click",d=>{d.preventDefault(),n.has(t)&&(URL.revokeObjectURL(n.get(t)),n.delete(t)),v.delete(t),k.value="",S.value="",r.style.display="none",m.style.display="flex",i&&i("")}),s.getValue=()=>v.has(t)&&n.has(t)?n.get(t):S.value.trim(),s.setValue=d=>{const u=G(d);u?u.startsWith("blob:")?(g.src=u,x.textContent="📁 Local File (Pending Upload)",y.textContent="Selected from local disk",r.style.display="flex",m.style.display="none"):(n.has(t)&&(URL.revokeObjectURL(n.get(t)),n.delete(t)),v.delete(t),S.value=u,g.src=u,x.textContent="🌐 Image URL Attached",y.textContent=u,r.style.display="flex",m.style.display="none"):(n.has(t)&&(URL.revokeObjectURL(n.get(t)),n.delete(t)),v.delete(t),k.value="",S.value="",r.style.display="none",m.style.display="flex")},s}const be=e.querySelector("#passage-img-picker-container"),M=F("passage","Passage Image URL (Optional)","",()=>c());be.appendChild(M);const fe=e.querySelector("#question-img-picker-container"),P=F("question","Question Image URL (Optional)","",()=>c());fe.appendChild(P);const he=e.querySelector("#explanation-img-picker-container"),U=F("explanation","Explanation Image URL (Optional)","",()=>c());he.appendChild(U);let _=[];function H(t=[],l=[],a=[]){X.innerHTML="",_=[];for(let i=0;i<h;i++){const s=String.fromCharCode(65+i),o=document.createElement("div");o.style.cssText="background:var(--card-bg); border:1px solid var(--border-color); border-radius:8px; padding:10px; display:flex; flex-direction:column; gap:8px;",o.innerHTML=`
        <div style="display:flex; align-items:center; gap:10px;">
          <input type="radio" name="correct_opt_idx" value="${i}" ${q===i?"checked":""} style="width:18px; height:18px; cursor:pointer;" />
          <span style="font-weight:800; min-width:80px; font-size:0.9rem; color:var(--text-main);">Option ${s}:</span>
          <input type="text" class="form-control opt-en-input" data-idx="${i}" value="${t[i]||""}" placeholder="Option ${s} text in English" style="padding:6px 10px; flex:1;" />
          <input type="text" class="form-control opt-hi-input" data-idx="${i}" value="${l[i]||""}" placeholder="हिंदी विकल्प ${s}" style="padding:6px 10px; flex:1;" />
        </div>
        <div class="opt-img-picker-slot" style="margin-left:110px;"></div>
      `;const r=o.querySelector(".opt-img-picker-slot"),g=F(`option_${i}`,`Option ${s} Image URL (Optional)`,a[i]||"",()=>c());r.appendChild(g),_.push(g),X.appendChild(o)}X.querySelectorAll("input").forEach(i=>{i.addEventListener("input",c),i.addEventListener("change",c)})}xe.addEventListener("click",()=>{if(h<6){h++;const t=Array.from(e.querySelectorAll(".opt-en-input")).map(i=>i.value),l=Array.from(e.querySelectorAll(".opt-hi-input")).map(i=>i.value),a=_.map(i=>i.getValue());H(t,l,a),c()}else alert("Maximum 6 options allowed per question.")}),ye.addEventListener("click",()=>{if(h>2){h--,q>=h&&(q=0);const t=Array.from(e.querySelectorAll(".opt-en-input")).map(i=>i.value),l=Array.from(e.querySelectorAll(".opt-hi-input")).map(i=>i.value),a=_.map(i=>i.getValue());H(t,l,a),c()}else alert("Minimum 2 options required per question.")}),C.addEventListener("click",()=>{f="en",C.classList.add("active"),O.classList.remove("active"),C.style.borderBottom="3px solid var(--primary)",O.style.borderBottom="none",K.style.display="block",ee.style.display="none",te.style.display="block",ae.style.display="none",ie.style.display="block",oe.style.display="none",se.textContent="English",c()}),O.addEventListener("click",()=>{f="hi",O.classList.add("active"),C.classList.remove("active"),O.style.borderBottom="3px solid var(--accent)",C.style.borderBottom="none",ee.style.display="block",K.style.display="none",ae.style.display="block",te.style.display="none",oe.style.display="block",ie.style.display="none",se.textContent="Hindi (हिंदी)",c()}),ge.addEventListener("click",()=>I(T));try{const l=(await z("/categories").catch(()=>({flatCategories:[]}))).flatCategories||[],a=l.filter(o=>!o.institute_id||o.is_global),i=l.filter(o=>o.institute_id&&!o.is_global);let s='<option value="">-- Select Category --</option>';a.length>0&&(s+='<optgroup label="🌐 Global Master Categories">'+a.map(o=>`<option value="${o.id}">${o.icon||"📂"} ${o.name}</option>`).join("")+"</optgroup>"),i.length>0&&(s+='<optgroup label="🏫 Institute Private Categories">'+i.map(o=>`<option value="${o.id}">${o.icon||"📂"} ${o.name}</option>`).join("")+"</optgroup>"),j.innerHTML=s}catch(t){console.warn("Could not load categories:",t)}function c(){ve.textContent=J.value||"Medium";const t=f==="hi"&&B.value||$.value,l=M.getValue(),a=e.querySelector("#prevPassageBox"),i=e.querySelector("#prevPassageText"),s=e.querySelector("#prevPassageImg");t.trim()||l?(a.style.display="block",i.innerHTML=t,l?(s.src=l,s.style.display="block"):s.style.display="none"):a.style.display="none";const o=f==="hi"&&Q.value||R.value,r=e.querySelector("#prevQText"),g=e.querySelector("#prevQImg");r.innerHTML=o||`Type a question statement in ${f==="hi"?"Hindi":"English"}...`;const x=P.getValue();x?(g.src=x,g.style.display="block"):g.style.display="none";const y=e.querySelector("#prevOptionsContainer");y.innerHTML="",e.querySelectorAll('input[name="correct_opt_idx"]').forEach(p=>{p.checked&&(q=parseInt(p.value,10))});const m=Array.from(e.querySelectorAll(".opt-en-input")).map(p=>p.value),S=Array.from(e.querySelectorAll(".opt-hi-input")).map(p=>p.value),k=_.map(p=>p.getValue());for(let p=0;p<h;p++){const E=p===q,qe=f==="hi"&&S[p]||m[p],de=k[p]||"",N=document.createElement("div");N.className=`option-btn ${E?"selected":""}`,N.style.cssText=`padding: 10px 14px; border: 2px solid ${E?"var(--primary)":"var(--border-color)"}; background: ${E?"var(--primary-light)":"var(--card-bg)"}; border-radius: var(--radius-md); font-weight: 600; display:flex; flex-direction:column; gap:6px;`,N.innerHTML=`
        <div style="display:flex; align-items:center; gap:10px;">
    ...
    `).join(""),t.querySelectorAll(".btn-remove-tag-chip").forEach(l=>{l.addEventListener("click",a=>{a.preventDefault();const i=parseInt(l.dataset.idx,10);w.splice(i,1),V()})})}}function re(){const t=e.querySelector("#input-new-tag");if(!t)return;const l=t.value.trim();if(!l)return;l.split(",").map(i=>i.trim()).filter(Boolean).forEach(i=>{w.includes(i)||w.push(i)}),t.value="",V()}const le=e.querySelector("#btn-add-tag-chip"),ne=e.querySelector("#input-new-tag");if(le&&le.addEventListener("click",re),ne&&ne.addEventListener("keydown",t=>{(t.key==="Enter"||t.key===",")&&(t.preventDefault(),re())}),V(),H(),b)try{const a=((await z("/exams/questions/all")).questions||[]).find(i=>i.id==b);if(a){R.value=a.question_text_en||a.question_text||"",Q.value=a.question_text_hi||"",A.value=a.explanation_en||a.explanation||"",D.value=a.explanation_hi||"",J.value=a.difficulty||"medium",a.category_id&&(j.value=a.category_id),W&&(W.checked=!!a.is_global),a.passage_text_en&&($.value=a.passage_text_en),a.passage_text_hi&&(B.value=a.passage_text_hi),a.passage_image_url&&M.setValue(a.passage_image_url),a.image_url&&P.setValue(a.image_url),a.explanation_image_url&&U.setValue(a.explanation_image_url),Array.isArray(a.tags)&&a.tags.length>0?w=[...a.tags]:a.tag_names&&(w=a.tag_names.split(",").map(r=>r.trim()).filter(Boolean)),V();const i=a.options_en||a.options||[],s=a.options_hi||[],o=(a.options_images||[]).map(G);h=Math.max(2,Math.min(6,i.length||4)),q=a.correct_option_index!==void 0?a.correct_option_index:a.correct_answer_index||0,H(i,s,o),c()}}catch(t){console.error("Failed loading question details:",t)}else c();async function pe(t=!1){const l=R.value.trim();if(!l){alert("Question Statement in English is required.");return}const a=Array.from(e.querySelectorAll(".opt-en-input")).map(r=>r.value.trim()),i=Array.from(e.querySelectorAll(".opt-hi-input")).map(r=>r.value.trim()),s=_.map(r=>r.getValue()),o={category_id:j.value?parseInt(j.value,10):null,difficulty:J.value,passage_text_en:$.value.trim(),passage_text_hi:B.value.trim(),passage_image_url:M.getValue(),question_text_en:l,question_text_hi:Q.value.trim(),image_url:P.getValue(),options_en:a,options_hi:i,options_images:s,correct_option_index:q,explanation_en:A.value.trim(),explanation_hi:D.value.trim(),explanation_image_url:U.getValue(),is_global:W?W.checked:!1,tags:w};if(v.size>0){ke("Uploading Image Assets...",`Uploading ${v.size} image asset(s) to server...`);try{for(const[r,g]of v.entries()){const x=new FormData;x.append("image",g);const y=await z("/images/upload",{method:"POST",body:x}),L=G(y.imageUrl||y.fullUrl);if(r==="passage")o.passage_image_url=L;else if(r==="question")o.image_url=L;else if(r==="explanation")o.explanation_image_url=L;else if(r.startsWith("option_")){const m=parseInt(r.replace("option_",""),10);!isNaN(m)&&m<o.options_images.length&&(o.options_images[m]=L)}}}catch(r){ce(),alert("Failed to upload image assets: "+r.message);return}finally{ce()}}try{b?await z(`/exams/questions/${b}`,{method:"PUT",body:JSON.stringify(o)}):await z("/exams/questions",{method:"POST",body:JSON.stringify(o)}),n.forEach(r=>URL.revokeObjectURL(r)),n.clear(),v.clear(),t?(alert("Master question saved successfully!"),R.value="",Q.value="",A.value="",D.value="",$.value="",B.value="",M.setValue(""),P.setValue(""),U.setValue(""),w=[],V(),h=4,q=0,H(),c()):I(T)}catch(r){alert(r.message||"Error saving master question.")}}me.addEventListener("click",()=>pe(!1)),ue.addEventListener("click",()=>pe(!0))}export{Ie as renderMasterQuestionEditorView};
