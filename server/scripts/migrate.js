...
export async function runMigrations() {
  ...
  try {
    ...
    await addColumnSafely('institutes', 'bunny_library_id', 'INT NULL');
    await addColumnSafely('institutes', 'bunny_api_key', 'VARCHAR(255) NULL');
    await addColumnSafely('institutes', 'bunny_token_key', 'VARCHAR(255) NULL');
    await addColumnSafely('institutes', 'bunny_cdn_hostname', 'VARCHAR(255) NULL');
    await addColumnSafely('institutes', 'video_quota_gb', 'INT DEFAULT 50');
    ...
    // -------------------------------------------------------------
    // ENHANCEMENT TABLES (STUDY MATERIALS, VIDEOS, LIVE CLASSES)
    // -------------------------------------------------------------
    console.log('📚 Initializing Study Materials, Video Lectures & Live Class Tables...');
    ...
    await conn.query(`
      CREATE TABLE IF NOT EXISTS video_lectures (
        ...
        status ENUM('uploading', 'processing', 'ready', 'failed', 'no_broadcast') DEFAULT 'uploading',
        is_all_batches BOOLEAN DEFAULT FALSE,
    ...
    await addColumnSafely('live_classes', 'stream_id', 'VARCHAR(100) NULL');
    await addColumnSafely('live_classes', 'rtmp_url', 'VARCHAR(500) NULL');
    ...
    await addIndexSafely('study_materials', 'idx_sm_inst', 'institute_id');
    await addIndexSafely('video_lectures', 'idx_vl_inst', 'institute_id');
    await addIndexSafely('live_classes', 'idx_lc_inst', 'institute_id');
    await addIndexSafely('live_classes', 'idx_lc_status', 'status');
    await addIndexSafely('video_watch_history', 'idx_vwh_user', 'user_id');
